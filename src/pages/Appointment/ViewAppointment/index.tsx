...
const ViewAppoinment: React.FC = (): JSX.Element => {
  const { id = '' } = useParams();
  ...
  const onEditAppoinment = () => {
    navigate(getEditAppointmentRoute(id));
  };

  const { mutate: deleteAppoinment, isPending: isDeleteInProgress } =
    useDeleteAppointment({
      onSuccess: () => {
        navigate(APPOINTMENTS, {
          state: {
            alert: {
              ...
              message: `Appoinment "${deleteConfirmationModalValues?.name}" is deleted successfully.`,
            },
          },
        });
      },
  ...
  const {
    ...
    onClose,
  } = useDeleteConfirmationModal({ onDelete: deleteAppoinment });
